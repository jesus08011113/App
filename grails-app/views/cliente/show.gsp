	...
	<body>
		...
		<div id="show-cliente" class="content scaffold-show" role="main">
			...
			<ol class="property-list cliente">
				...
				<li class="fieldcontain">
					<span id="servicios-label" class="property-label"><g:message code="cliente.servicios.label" default="Servicios" /></span>
					
						<g:each in="${clienteInstance.servicios}" var="s">
						<span class="property-value" aria-labelledby="servicios-label"><g:link controller="os" action="show" id="${s.id}">${s?.encodeAsHTML()}</g:link></span>
						</g:each>
