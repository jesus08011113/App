...
<center>
  ...
  <table>
    ...
    <tr>
      <td class="fieldcontain ${hasErrors(bean: osInstance, field: 'recomendaciones', 'error')} ">
    ...
    <tr>
      <td >
         <g:textArea name="recomendaciones" value="${osInstance?.recomendaciones}"/>
      </td>
    </tr>
  ...
  <table>
    <tr>
      ...
      <td class="fieldcontain ${hasErrors(bean: osInstance, field: 'entrega', 'error')} required">
        <label for="entrega">
                <span class="required-indicator">*</span>
		<g:message code="os.entrega.label" default="Entrega el equipo:" />
    ...
    <tr>
      ...
      <td>
        <g:select id="entrega" name="entraga.id" from="${ingenierosLista}" optionKey="id" required="" value="${osInstance?.entrega?.id}" class="many-to-one"  noSelection="['':'Entrega a cliente']"/>
      </td>
